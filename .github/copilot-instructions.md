...
- Put implementation functions in `lib/<command>` (for example `lib/code`, `lib/c`).
- Every function must live in its own file inside the relevant `lib/<command>` folder.
...
- You may create subfolders in a `lib/<command>` folder when grouping improves clarity.
- Keep a concise `readme.md` in each `lib/<command>` folder as a cheat sheet for the layer structure and key files.
- When working in a `lib/<command>` folder, quickly read that folder's `readme.md` first to understand the implementation layout.
- When changing the structure or responsibilities in a `lib/<command>` folder, keep its `readme.md` up to date.
